...
#                                 Point Structure
#
# Point structure allows for a more naturally dynamic token metadat through an
...
#
# This is useful when the metadata depends on some properties (of the entity
# behind the token) that are stored in the contract's storage.
#
# The liveness of the external server on which the metadata is hosted must be
# ensured. It is recommended to use a decentralised storage service like IPFS.
# Moreoever, the URI generated by the off-chain view must be verified.
#################################################################################
